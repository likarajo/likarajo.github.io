...
Without this tool in hand, the only way to check the request messages was to dig into the huge set of application logs present in the application server (Linux).
Moreover, the full set of logs in all the 6 application production servers had to be parsed through individually, which itself being tremendously hectic would also consume a high amount of time and reduce team members' productivity, ultimately delaying monitoring.
