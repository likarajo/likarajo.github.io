...
function Articles() {
    ...
    return (
        <div style={{width: "60%", minWidth: "280px", paddingBottom: "20px", margin: "10px auto"}}>
            <Stack direction="row" alignItems="center"><Typography variant="h5">Cities and Landmarks</Typography>&nbsp;({filtered.length})</Stack>
                ...
                {filtered?.map((item) => (
                    <a key={item.title} href={item.link} target={item.link.includes("http")?"_blank":"_self"}>
                        <ImageListItem style={{width: "100%", aspectRatio: "4/3"}}>
                            ...
                            <ImageListItemBar
                                title={
                                    <Typography variant={isDesktop?"h6":"span"}><b>{item.title}</b></Typography>
                                }
                                subtitle={
